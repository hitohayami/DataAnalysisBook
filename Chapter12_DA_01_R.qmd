...
if( length(grep('mac',RInfor))>0) {Kanji="HiraginoSans-W3"}
```
...
```{r}
#| label: Read_Data
url = "https://raw.githubusercontent.com/hitohayami/DataAnaysisBook/main/DisneyPrice.csv"
...
#| label: Simple_Graph
par(family = Kanji)
stock_xts <- xts(df[, c("Close","High","Low","Open","Volume")], order.by = df$Date)
chartSeries(stock_xts, name = "Tokyo Disney", theme = chartTheme("white"),TA=c(addVo(),addBBands(),addMACD()),cex=0.3)
```

```{r}
#| label: ShrinkageTimeVaryingParameter
DisneyJ=specifyModel(Next(OpCl(stock_xts)) ~ Lag(OpHi(stock_xts),0:3))
m.DisneyJ=buildModel(DisneyJ,method='lm',training.per=c('2007-01-01','2023-05-31'))
summary(m.DisneyJ)
(t.DisneyJ=tradeModel(m.DisneyJ))

par(family = Kanji)
plot(monthlyReturn(stock_xts))

# Data for the estimation
SP_OLand=data.frame(OpCl(stock_xts),OpHi(stock_xts))
...
# Hit <Return>
plot(ShrinkTVP_OL,family=Kanji)
...
YJ_mat=as.matrix(stock_xts,col=6)
x_time=time(stock_xts)
Max_T=dim(x_time)[1]
rm(YJ_mat)
...
id_inf=which(is.infinite(x[,2]))
x=x[-id_inf,]
x_time=x_time[-id_inf]
n=length(x_time)
...
hist(x_1$dy,nclass=50,main="",xlab="Change Rate of Stock Price")
...
hist(x_2$dy,nclass=50,main="",xlab="Change Rate of Stock Price")
